...
initLocalStream() {
    ...
    let socket = io('http://indely.net:5000');

    socket.emit('subscribe', this.localId);

    atom.clipboard.write('http://indely.net/#/'+this.localId);
    atom.notifications.addInfo('Copied url Classroom into clipboard');

    socket.on('chat', this.indelyView.addMessage);
